...
old_jsx_regex = r'<div className="p-6 space-y-10">.*?</div>\s*\{\/\* Hidden Print Container \*\/\}'
...
content = re.sub(old_jsx_regex, new_jsx, content, flags=re.DOTALL)
...
old_print_regex = r'<div id="twtw-demo-print-area" className="hidden">.*?</div>\s*</CardContent>'
print_container = """<div id="twtw-demo-print-area" className="hidden" dangerouslySetInnerHTML={{ __html: `
                ${Object.entries(collatedMockSubmissions).map(([state, subs]) => `
...
content = re.sub(old_print_regex, print_container, content, flags=re.DOTALL)
