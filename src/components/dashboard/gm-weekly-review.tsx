...
export function GMWeeklyReview({ week: propWeek }: { week?: string }) {
  ...
  const handleDispatchToGM = async () => {
    ...
    // Force overview tab to ensure individual results are rendered
    setSelectedTab('overview');
    
    // Slight delay to allow DOM to render the overview tab fully if it was switched
    setTimeout(async () => {
      try {
        const element = document.getElementById('gm-report-capture');
        if (!element) throw new Error("Capture element not found");
        
        toast({ title: "Generating PDF", description: "Compiling High-Res GM Report..." });
        ...
        toast({ title: "Dispatch Complete", description: "PDF downloaded successfully." });
      } catch (err) {
        console.error(err);
...
function BDMReportCard({ report, onSaveFeedback }: { report: BDMWeeklyReport; onSaveFeedback: (uid: string, f: string) => void }) {
  const [feedback, setFeedback] = useState('');
  const [isOpen, setIsOpen] = useState(false);

  return (
    <Card className="border-none shadow-lg bg-white overflow-hidden transition-all hover:shadow-xl">
      <div className="p-6">
        <div className="flex items-center justify-between mb-6">
          ...
          <Button variant="ghost" onClick={() => setIsOpen(!isOpen)} className="text-accent font-black text-[10px] uppercase">
